...
\begin{document}
	...
	\paragraph{Removal of outliers}
	TODO: code this - fix this?
	To properly be able to read the elite run graphs, all high TTE outliers are removed. This however is not done in i.e the cost saving graphs, as the outliers don't make the graph less readable. This causes some experiment data to 
	...
	\end{verbatim}
	TODO: clean up amount of elites in cost saving vs elite run
	Experiment 2 achieved 1.74\% reduction in optimal query times, in 138 generations before converging, being the experiment that took the longest to converge. In this experiment the first goal defining default run did not spike as high as in the other experiments, leading to a interesting cost saving graph \ref{fig:final22} that shortly dips below 0\%. Furthermore there is a tremendous dip in cost saving at elite \#number, which coincides with the elite run graph \ref{fig:final23}. The elite run graph shows a couple of high TTEs within the span of the first 10 generations, one major spike, before oscillating around 9.1. Viewing the solution assurance graph \ref{fig:final24} shows that the optimal solution repeatedly beats the default candidate, even when experiencing variations and oscillations.
	\clearpage
	\begin{figure}[H]
		\centering
		\includegraphics[width=140pt]{runlogs/final2/1}
		\caption{Experiment 2: Total probability convergence over generations}
		\label{fig:final21}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=140pt]{runlogs/final2/2}
		\caption{Experiment 2: Cost saving of optimal elite run vs default benchmark}
		\label{fig:final22}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=140pt]{runlogs/final2/3}
		\caption{Experiment 2: Elite run graph, showing elites optimal TTE over generations}
		\label{fig:final23}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=140pt]{runlogs/final2/4}
