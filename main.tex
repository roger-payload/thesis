...
\begin{document}
	...
	-------------  ------------------  ---------  ----------------
	Optimal query             9.19285    9.49949  3.23%
		
	\end{verbatim}
	Experiment 1 achieved 3.23\% reduction in optimal query times, in 128 generations before converging. The real cost saving is however expected to be a bit lower, as data from the first default run is included in the calculations - and as you can see from figure \ref{fig:final14}, it is a high outlier. This is one of the runs discussed in the section about \hyperref[sec:agnosstable]{accelerated convergence}, because of the long time it took for elites to reach consensus on parameter options, as shown in figure \ref{fig:final11}. It initially rises tremendously in total convergence up until generation 20, before spending over 100 generations to settle for the remaining parameters. The cost saving measured in figure \ref{fig:final12} is also compared to the default outlier, as that was the run defining the goal benchmark. In relation to the convergence graph \ref{fig:final11} we can see why convergence after generation 20 is hard to reach, as the cost saving flatlines. Figure \ref{fig:final13} shows the optimal query times for each elite gathered from each generation. It curves downwards nicely, but it is clear the first few elites generated were bad solutions, with TTE of around 9.40. Each generation had 3 elites added, meaning that according to the fitness and convergence charts, the elites TTE should balance out at around generation 20, which here will be seen as elite \#60. This is confirmed by looking at the spread, which keeps repeating results between 9.10 and 9.20 from around elite 60\# onward. Finally figure \ref{fig:final14} shows the solution assurance runs. Even though run 1 for the default candidate has a major spike in TTE, our optimal solution repeatedly performs better in the 10 subsequent runs performed during the solution assurance stage.
	\clearpage
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final1/1}
		\caption{Experiment 1: Total convergence values over time}
		\label{fig:final11}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final1/2}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final12}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final1/3}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final13}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final1/4}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final14}
	\end{figure}
	\clearpage
	...
	Metric           Optimal solution    Default  Overhead saved
	-------------  ------------------  ---------  ----------------
	Optimal query             9.15607     9.3482  2.06%
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final2/1}
		\caption{Experiment 1: Total convergence values over time}
		\label{fig:final21}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final2/2}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final22}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final2/3}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final23}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final2/4}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final24}
	\end{figure}
	\clearpage
	...
	Convergence threshold reached at generation 136
	
	
	Results:
	Metric           Optimal solution    Default  Overhead saved
	-------------  ------------------  ---------  ----------------
	Optimal query             9.14703    9.35349  2.21%
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final3/1}
		\caption{Experiment 1: Total convergence values over time}
		\label{fig:final31}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final3/2}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final32}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final3/3}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final33}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=145pt]{runlogs/final3/4}
		\caption{All the technological layers that we pass during a single query}
		\label{fig:final34}
	\end{figure}
	\clearpage
	...
	\section{Parameter agnostic queries}
		...
		\begin{figure}[ht]
			\centering
			\subfloat[label 1]{{\includegraphics[width=5cm]{agnosticism2} }}
			\qquad
			\subfloat[label 2]{{\includegraphics[width=5cm]{agnosfit2} }}
	...
	\section{Proving effect}
	 After analyzing our data it is clear that the algorithm works as intended - reducing the overall TTE. Although some error sources will put individual runs from the default candidate ahead of the considered optimal solution in terms of TTE, it is proven across several solution assurance runs that the average cost saving of the suggested optimal solution is worthwhile, averaging out at 3\% for the \hyperref[table:cluster_stable]{stable cluster}. For the \hyperref[table:cluster_shared]{shared cluster} cost saving is inconclusive, because of the heavy variance in TTE caused by the shared resources, leading to faulty results.
